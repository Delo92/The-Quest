...
export async function registerRoutes(
  ...
  /* ── CBUSA Partner Pipeline: Launchpad Enrollment Webhook ─────────────────
     CBUSA (parent company) POSTs here after an artist pays on their platform.
     We create the artist's account, enroll them in the competition, and send
     a welcome email with credentials. Returns 200 so CBUSA flips
     cbpublishingSynced = true in their Firestore.
  ─────────────────────────────────────────────────────────────────────────── */
  app.post("/api/webhooks/launchpad-enrollment", async (req, res) => {
    // 1. Verify shared secret
    const apiKey = req.headers["x-api-key"];
    const expectedKey = process.env.PROMO_API_KEY;
    if (!expectedKey || apiKey !== expectedKey) {
      console.warn("[LaunchpadWebhook] Rejected — invalid or missing x-api-key");
    ...
    const siteUrl = process.env.SITE_URL ? `https://${process.env.SITE_URL}` : `https://${req.get("host")}`;
    const displayName = `${firstName} ${lastName || ""}`.trim();

    try {
      // 2. Create or retrieve Firebase Auth account (idempotent — safe to retry)
      let firebaseUser: any;
      let tempPassword: string | null = null;
      let isNewAccount = false;

      try {
        firebaseUser = await getFirebaseAuth().getUserByEmail(email.toLowerCase().trim());
        console.log(`[LaunchpadWebhook] Existing account for ${email} (uid: ${firebaseUser.uid})`);
      } catch (lookupErr: any) {
        if (lookupErr.code === "auth/user-not-found") {
          ...
          firebaseUser = await createFirebaseUser(email, tempPassword, artistName || displayName);
          await setUserLevel(firebaseUser.uid, 2); // level 2 = talent
          isNewAccount = true;
          console.log(`[LaunchpadWebhook] Created new account for ${email} (uid: ${firebaseUser.uid})`);
        } else {
          throw lookupErr;
        }
      }

      // 3. Create Firestore user record (skip if account already existed)
      if (isNewAccount) {
        await createFirestoreUser({
          ...
          displayName,
          level: 2,
          stageName: artistName || undefined,
        });
      }

      // 4. Build or update talent profile
      let talentProfile = await firestoreTalentProfiles.getByUserId(firebaseUser.uid);
      if (!talentProfile) {
        const socialLinksJson = instagramHandle
          ? JSON.stringify({ instagram: instagramHandle })
          : null;
        ...
        const createdTalentProfile = await (firestoreTalentProfiles.create as any)({
          ...
          email: email.toLowerCase().trim(),
          bio: null,
          category: competitionCategory || genre || null,
          location: city || null,
          imageUrls: [],
          videoUrls: [],
          socialLinks: socialLinksJson,
          role: "talent",
          // Attribution fields stored alongside the standard profile
          launchpadEnrollment: {
            promoCode: promoCode || null,
            partnerStudio: partnerStudio || null,
            source: source || null,
            market: market || null,
            orderId: orderId || cbusaOrderId || null,
            cbusaTransactionId: cbusaTransactionId || null,
            enrollPackage: enrollPackage || null,
      ...
      let matchedCompetition: any = null;
      if (competition || competitionCategory) {
        const allComps = await storage.getCompetitions();
        const searchTitle = (competition || "").toLowerCase().replace(/\s+/g, "");
        const searchCat = (competitionCategory || genre || "").toLowerCase();

        matchedCompetition = allComps.find((c: any) => {
          const titleNorm = (c.title || "").toLowerCase().replace(/\s+/g, "");
          return titleNorm === searchTitle || titleNorm.includes(searchTitle) || searchTitle.includes(titleNorm);
        }) || allComps.find((c: any) => {
          return (c.category || "").toLowerCase() === searchCat && c.status !== "completed";
        });
      }
      ...
      if (matchedCompetition) {
        ...
        const existing = await firestoreContestants.get(matchedCompetition.id, talentProfile.id);
        if (!existing) {
          contestant = await storage.createContestant({
            competitionId: matchedCompetition.id,
            talentProfileId: talentProfile.id,
            applicationStatus: "approved",
            appliedAt: purchasedAt || new Date().toISOString(),
          });
          console.log(`[LaunchpadWebhook] Enrolled contestant ${contestant.id} in competition "${matchedCompetition.title}"`);
        } else {
          contestant = existing;
          console.log(`[LaunchpadWebhook] Already enrolled in competition "${matchedCompetition.title}" — skipping`);
        }
      } else {
        console.warn(`[LaunchpadWebhook] No matching competition found for "${competition}" / category "${competitionCategory}"`);
      }

      // 6. Send welcome email (non-blocking if email not yet configured)
      if (isNewAccount && tempPassword) {
        if (isEmailConfigured()) {
          const steps: string[] = Array.isArray(nextSteps) && nextSteps.length > 0
            ? nextSteps
            : [
                "Complete your artist bio on your profile",
                "Upload your gallery photos",
                "Upload your music/video content — it publishes to ChronicTV with your vote link",
                "Share your vote link with your fans",
              ];

          sendLaunchpadWelcomeEmail({
            to: email.toLowerCase().trim(),
            firstName,
            artistName: artistName || displayName,
            email: email.toLowerCase().trim(),
            tempPassword,
            competitionName: matchedCompetition?.title || competition || "The Quest",
            partnerStudio: partnerStudio || "CB Publishing",
            nextSteps: steps,
            siteUrl,
          }).catch((err: any) => {
            console.warn("[LaunchpadWebhook] Welcome email failed (non-blocking):", err.message);
          });
        } else {
          console.warn("[LaunchpadWebhook] Email not configured — skipping welcome email (GMAIL_CLIENT_SECRET missing?)");
        }
      }

      // 7. Return 200 so CBUSA flips cbpublishingSynced = true
      return res.status(200).json({
        success: true,
        uid: firebaseUser.uid,
        talentProfileId: talentProfile.id,
        contestantId: contestant?.id || null,
        competitionTitle: matchedCompetition?.title || null,
        isNewAccount,
      });

    } catch (err: any) {
      console.error("[LaunchpadWebhook] Unhandled error:", err.message, err.stack);
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    try {
      ...
      // Notify referral code owner when their code was used during sign-up (non-blocking)
      if (referralCode?.trim() && isEmailConfigured()) {
        (async () => {
          try {
            const resolvedRef = referralCode.trim().toUpperCase();
            const codeDoc = await firestoreReferrals.getCodeByCode(resolvedRef);
            if (codeDoc) {
              let ownerEmail = codeDoc.ownerEmail || null;
              if (!ownerEmail && codeDoc.ownerId) {
                try {
                  const fbUser = await getFirebaseAuth().getUser(codeDoc.ownerId);
                  ownerEmail = fbUser.email || null;
                } catch {}
              }
              if (ownerEmail) {
                const siteUrl = process.env.SITE_URL || `https://${req.get("host")}`;
                await sendCodeUsedNotification({
                  to: ownerEmail,
            ...
            console.warn("[CodeNotify] Sign-up referral owner notification failed (non-blocking):", err.message);
          }
        })();
      }
  ...
  app.get("/api/competitions/:competitionId/contestants/:contestantId/referral-code", async (req, res) => {
    try {
      ...
      // Previous versions created an unbranded 8-character code for this lookup.
      // Keep those old records for historical attribution, but don't present them
      // as the contestant's chosen code on a new registration.
      const matchingCode = activeCodes.find((code) =>
        (code.competitionId === competitionId || code.competitionIds?.includes(competitionId))
        && !/^[A-F0-9]{8}$/i.test(code.code)
      );
      if (matchingCode) {
        return res.json({ code: matchingCode.code });
      }

      if (talentProfile?.userId) {
        const ownerCodes = (await firestoreReferrals.getCodesByOwner(talentProfile.userId))
          .filter((code) => !code.aliasFor);
        const isLegacyAutoGeneratedCode = (code: typeof ownerCodes[number]) =>
          code.contestantId === contestantId && /^[A-F0-9]{8}$/i.test(code.code);
        const ownerCode = ownerCodes.find((code) =>
          !isLegacyAutoGeneratedCode(code)
          &&
          (!code.contestantId || code.contestantId === contestantId)
          && (code.competitionId === competitionId || code.competitionIds?.includes(competitionId))
        ) || ownerCodes.find((code) =>
          !isLegacyAutoGeneratedCode(code)
          &&
          !code.contestantId && !code.competitionId && !(code.competitionIds || []).length
        );
        if (ownerCode) return res.json({ code: ownerCode.code });
      }

      const competition = await storage.getCompetition(competitionId);
      if (!competition) return res.json({ code: null });
      const competitionReferral = await getOrCreateCompetitionReferralCode(competition);
      res.json({ code: competitionReferral?.code || null });
    } catch (error: any) {
  ...
  app.get("/api/host/competitions/:id/report", firebaseAuth, requireHost, async (req, res) => {
    ...
    const [contestants, purchases, allSubmissions] = await Promise.all([
      storage.getContestantsByCompetition(id),
      storage.getVotePurchasesByCompetition(id),
      firestoreJoinSubmissions.getAll(),
    ]);
    ...
    const voteRevenue = purchases.reduce((sum, p) => sum + p.amount, 0);

    const compNominations = allSubmissions.filter(s => s.type === "nomination" && String(s.competitionId) === String(id));
    const nominationRevenue = compNominations.reduce((sum, s) => sum + (s.amountPaid || 0), 0);
    const paidNominations = compNominations.filter(s => s.amountPaid && s.amountPaid > 0).length;
    ...
    res.json({
      ...
      voteRevenue,
      nominationRevenue,
      totalRevenue: voteRevenue + nominationRevenue,
      totalNominations: compNominations.length,
      paidNominations,
  ...
  app.get("/api/admin/competitions/:id/report", firebaseAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const [contestantsData, totalVotes, purchases, allSubmissions] = await Promise.all([
        storage.getContestantsByCompetition(id),
        storage.getTotalVotesByCompetition(id),
        storage.getVotePurchasesByCompetition(id),
        firestoreJoinSubmissions.getAll(),
  ...
  app.post("/api/join/nominate", async (req, res) => {
    try {
      ...
      // Track referral code (recruit link) nomination with Chronic Brands USA + internal stats
      if (referralCode?.trim()) {
        ...
        // Notify the code owner by email (non-blocking)
        if (isEmailConfigured()) {
          (async () => {
            try {
              const codeDoc = await firestoreReferrals.getCodeByCode(resolvedRef);
              ...
              console.warn("[CodeNotify] Nomination referral owner notification failed (non-blocking):", err.message);
            }
          })();
        }
      }
  ...
  app.post("/api/guest/checkout", async (req, res) => {
    try {
      ...
      if (resolvedRefCode) {
        ...
        // Track referral code redemption with Chronic Brands USA (non-blocking)
        trackChronicBrandsPromo({
  ...
  // ── Analytics Routes ─────────────────────────────────────────────
  app.get("/api/analytics/nominations", firebaseAuth, requireAdmin, async (req, res) => {
    try {
      const [allSubmissions, competitions] = await Promise.all([
        firestoreJoinSubmissions.getAll(),
        storage.getCompetitions(),
      ]);
      const compMap: Record<number, string> = {};
      for (const c of competitions) compMap[c.id] = c.title;

      const nominations = allSubmissions
        .filter(s => s.type === "nomination")
        .map(s => ({
          id: s.id,
          createdAt: s.createdAt,
          nomineeName: s.fullName,
          nomineeEmail: s.email,
          nominatorName: s.nominatorName || null,
          nominatorEmail: s.nominatorEmail || null,
          competitionId: s.competitionId || null,
          competitionTitle: s.competitionId ? (compMap[s.competitionId] || `Competition #${s.competitionId}`) : "No competition",
          amountPaid: s.amountPaid || 0,
          transactionId: s.transactionId || null,
          referralCode: (s as any).referralCode || null,
        }))
  ...
  app.get("/api/analytics/overview", firebaseAuth, requireAdmin, async (req, res) => {
    try {
      const [competitions, allContestants, allSubmissions] = await Promise.all([
        storage.getCompetitions(),
        storage.getAllContestants(),
        firestoreJoinSubmissions.getAll(),
      ]);

      // Index nomination fees by competitionId
      const nomByComp: Record<string | number, { revenue: number; count: number; paid: number }> = {};
      let totalNominationRevenue = 0;
      let totalNominationCount = 0;
      let totalPaidNominations = 0;
      for (const sub of allSubmissions) {
        if (sub.type !== "nomination") continue;
        const cid = sub.competitionId ?? "none";
        if (!nomByComp[cid]) nomByComp[cid] = { revenue: 0, count: 0, paid: 0 };
        nomByComp[cid].count++;
        totalNominationCount++;
        if (sub.amountPaid && sub.amountPaid > 0) {
          nomByComp[cid].revenue += sub.amountPaid;
          nomByComp[cid].paid++;
          totalNominationRevenue += sub.amountPaid;
      ...
      for (const comp of competitions) {
        ...
        const voteRevenue = purchases.reduce((sum, p) => sum + (p.amount || 0), 0);
        totalVoteRevenue += voteRevenue;

        const nomData = nomByComp[comp.id] || { revenue: 0, count: 0, paid: 0 };
        ...
        competitionStats.push({
          ...
          voteRevenue: voteRevenue / 100,
          nominationRevenue: nomData.revenue / 100,
          nominationCount: nomData.count,
          paidNominations: nomData.paid,
          revenue: (voteRevenue + nomData.revenue) / 100,
        });
      }
      ...
      res.json({
        ...
        totalOnline,
        totalInPerson,
        voteRevenue: totalVoteRevenue / 100,
        nominationRevenue: totalNominationRevenue / 100,
        totalRevenue: (totalVoteRevenue + totalNominationRevenue) / 100,
        totalNominations: totalNominationCount,
        paidNominations: totalPaidNominations,
        totalCompetitions: competitions.length,
