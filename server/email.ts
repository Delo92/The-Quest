...
/* ─────────────────────────────────────────────────────────────────────────
   7. PROMO / REFERRAL CODE USED  (sent to the code owner when their code is used)
───────────────────────────────────────────────────────────────────────── */
export async function sendCodeUsedNotification(opts: {
  to: string;
  ownerName: string;
  code: string;
  usedBy: string;
  usedByEmail: string;
  action: "nomination" | "signup";
  nomineeName?: string;
  competitionName?: string;
  siteUrl: string;
}): Promise<boolean> {
  try {
    ...
    const actionLine = opts.action === "nomination"
      ? `<strong style="color:#fff;">${opts.usedBy}</strong> just used your code to nominate <strong style="color:#FF5A09;">${opts.nomineeName || "someone"}</strong>${opts.competitionName ? ` for <strong style="color:#fff;">${opts.competitionName}</strong>` : ""}.`
      : `<strong style="color:#fff;">${opts.usedBy}</strong> just signed up using your referral code.`;

    const html = wrapInTemplate(`
      <div style="text-align:center; margin-bottom: 28px;">
        <span class="badge">Your Code Was Used!</span>
      </div>
      <h2 style="text-align:center;">Your Code Is Working, ${opts.ownerName}!</h2>
      <p style="text-align:center; font-size:16px; color:#ccc;">${actionLine}</p>

      <div class="highlight-box" style="text-align:center;">
        <p class="label">Your Code</p>
        <p class="value" style="font-family: monospace; font-size: 24px; letter-spacing: 4px; color: #FF5A09;">${opts.code}</p>
      </div>

      <div class="highlight-box">
        <p class="label">Used By</p>
        <p class="value">${opts.usedBy}</p>
        <p style="margin:4px 0 0; color:#555; font-size:13px;">${opts.usedByEmail}</p>
      </div>

      <p>Keep sharing your code — every use helps grow The Quest community and boosts your visibility on the platform.</p>

      <div class="btn-wrap">
        <a href="${opts.siteUrl}/thequest/talent-dashboard" class="btn">View My Dashboard</a>
      </div>
      <hr class="divider" />
      <p style="font-size:13px; color:#555;">Share your code with friends, fans, and on social media to maximize your reach!</p>
    `);

    await transporter.sendMail({
      from: `"${DISPLAY_NAME}" <${GMAIL_ADDRESS}>`,
      to: opts.to,
      subject: `Your code ${opts.code} was just used — The Quest`,
      html,
    });

    console.log(`Code-used notification sent to ${opts.to} (code: ${opts.code})`);
    return true;
  } catch (err: any) {
    console.error("Failed to send code-used notification:", err.message);
    resetTransporter();
    return false;
  }
}
...
/* ─────────────────────────────────────────────────────────────────────────
   LAUNCHPAD PARTNER ENROLLMENT WELCOME
   Sent when an artist is enrolled via the CBUSA partner pipeline.
───────────────────────────────────────────────────────────────────────── */
export async function sendLaunchpadWelcomeEmail(opts: {
  to: string;
  firstName: string;
  artistName: string;
  email: string;
  ...
  try {
    ...
    const nextStepsHtml = opts.nextSteps
      .map((step, i) => `<p style="margin:6px 0; color:#ccc; font-size:14px;"><span style="color:#FF5A09; font-weight:700;">${i + 1}.</span> ${step}</p>`)
      .join("");

    const html = wrapInTemplate(`
      <h2>Welcome to The Quest, ${opts.artistName}!</h2>
      <p style="color:#bbb; line-height:1.7;">
        You've been enrolled in <strong style="color:#fff;">${opts.competitionName}</strong> through
        <strong style="color:#fff;">${opts.partnerStudio}</strong>. Your account is ready — log in below to
        complete your profile and start collecting votes.
      </p>

      <div class="credentials-box">
        <p class="cred-label">Your Login Credentials</p>
        <p class="cred-row">Email: <span class="cred-value">${opts.email}</span></p>
        <p class="cred-row">Temporary Password: <span class="cred-value">${opts.tempPassword}</span></p>
        <p style="color:#555; font-size:12px; margin: 10px 0 0;">Change your password after your first login.</p>
      </div>

      <div class="highlight-box" style="margin-top:24px;">
        <p class="label" style="margin-bottom:10px;">Your Next Steps</p>
        ${nextStepsHtml}
      </div>

      <div class="btn-wrap" style="margin-top:28px;">
        <a href="${loginUrl}" class="btn">Log In &amp; Get Started</a>
      </div>
      <p class="fallback-link">Or copy this link: <a href="${loginUrl}">${loginUrl}</a></p>
    `);

    await transporter.sendMail({
      from: `"The Quest" <${GMAIL_ADDRESS}>`,
      to: opts.to,
      subject: `Welcome to The Quest — ${opts.competitionName} enrollment confirmed`,
      html,
    });

    console.log(`Launchpad welcome email sent to ${opts.to}`);
    return true;
  } catch (err: any) {
    console.error("Failed to send launchpad welcome email:", err.message);
    resetTransporter();
    return false;
  }
}
