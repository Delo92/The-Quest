...
export default function AdminAnalyticsTab() {
  ...
  const { data: allSubmissions, isLoading: submissionsLoading } = useQuery<RawSubmission[]>({
    queryKey: ["/api/admin/join/submissions"],
    queryFn: async () => {
      const token = await getAuthToken();
      const res = await fetch("/api/admin/join/submissions", {
        ...
        {analyticsLoading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <StatCard icon={UserPlus} label="Total Nominations" value={String(analytics.totalNominations)} color="text-purple-400" />
            </div>

            {/* Revenue stats */}
            <div className="rounded-md bg-white/5 border border-white/5 p-5">
              <h3 className="text-white font-semibold text-sm uppercase tracking-wider mb-4">Revenue Breakdown</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="rounded-md bg-white/[0.04] border border-white/8 p-4 text-center">
                  <DollarSign className="h-5 w-5 text-emerald-400 mx-auto mb-1" />
                  <p className="text-2xl font-bold text-emerald-400">${analytics.totalRevenue.toFixed(2)}</p>
                  <p className="text-xs text-white/40 uppercase tracking-wider mt-1">Total Revenue</p>
                </div>
                <div className="rounded-md bg-white/[0.04] border border-white/8 p-4 text-center">
                  <Vote className="h-5 w-5 text-blue-400 mx-auto mb-1" />
                  <p className="text-2xl font-bold text-blue-400">${(analytics.voteRevenue ?? 0).toFixed(2)}</p>
                  <p className="text-xs text-white/40 uppercase tracking-wider mt-1">Vote Pack Revenue</p>
                </div>
                <div className="rounded-md bg-white/[0.04] border border-white/8 p-4 text-center">
                  <UserPlus className="h-5 w-5 text-purple-400 mx-auto mb-1" />
                  <p className="text-2xl font-bold text-purple-400">${(analytics.nominationRevenue ?? 0).toFixed(2)}</p>
                  <p className="text-xs text-white/40 uppercase tracking-wider mt-1">Nomination Fees</p>
                  <p className="text-[10px] text-white/25 mt-0.5">{analytics.paidNominations} paid of {analytics.totalNominations} total</p>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10">
                      ...
                      <th className="text-right text-white/50 pb-3 pr-4 font-medium">Noms (Paid)</th>
                      <th className="text-right text-white/50 pb-3 pr-4 font-medium">Vote Rev.</th>
                      <th className="text-right text-white/50 pb-3 pr-4 font-medium">Nom. Rev.</th>
                    ...
                    {pagedComps.map(comp => (
                      <tr key={comp.id} className="border-b border-white/5 cursor-pointer" onClick={() => setSelectedCompetition({ id: comp.id, title: comp.title })} data-testid={`analytics-comp-${comp.id}`}>
                        ...
                        <td className="py-3 pr-4 text-right text-white/50">
                          {comp.nominationCount ?? 0}
                          {(comp.paidNominations ?? 0) > 0 && (
                            <span className="text-purple-400 ml-1">({comp.paidNominations})</span>
                          )}
                        </td>
                        <td className="py-3 pr-4 text-right text-blue-400">${(comp.voteRevenue ?? 0).toFixed(2)}</td>
                        <td className="py-3 pr-4 text-right text-purple-400">${(comp.nominationRevenue ?? 0).toFixed(2)}</td>
            ...
            {/* Nomination Payment Log */}
            <div className="rounded-md bg-white/5 border border-white/5 p-5">
              <h3 className="text-white font-semibold text-sm uppercase tracking-wider mb-1">Nomination Payment Log</h3>
              <p className="text-xs text-white/35 mb-4">Every nomination submitted — who nominated whom, for which competition, and what was paid.</p>
              {(() => {
                const compTitleMap: Record<number, string> = {};
                analytics?.competitionStats?.forEach((c: any) => { compTitleMap[c.id] = c.title; });
                const nominations = (allSubmissions ?? []).filter(
                  s => s.type === "nomination" || (!s.type && !!s.nominatorName)
                ).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
                if (submissionsLoading) return <p className="text-center text-white/30 py-6 text-sm">Loading...</p>;
                if (!allSubmissions) return <p className="text-center text-red-400/60 py-6 text-sm">Failed to load nomination data</p>;
                if (nominations.length === 0) return <p className="text-center text-white/30 py-6 text-sm">No nominations yet</p>;
                return (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-white/10">
                          ...
                          <th className="text-left text-white/50 pb-3 pr-4 font-medium">Nominee</th>
                          <th className="text-left text-white/50 pb-3 pr-4 font-medium">Nominated By</th>
                          <th className="text-left text-white/50 pb-3 pr-4 font-medium">Competition</th>
                          <th className="text-right text-white/50 pb-3 pr-4 font-medium">Amount</th>
                          <th className="text-left text-white/50 pb-3 pr-4 font-medium">Code Used</th>
                          <th className="text-left text-white/50 pb-3 font-medium">Transaction ID</th>
                        </tr>
                      </thead>
                      <tbody>
                        {nominations.map((n, i) => (
                          <tr key={n.id} className="border-b border-white/5" data-testid={`nom-payment-${i}`}>
                            <td className="py-3 pr-4 text-white/40 text-xs whitespace-nowrap">
                              {new Date(n.createdAt).toLocaleDateString()}<br />
                              <span className="text-white/25">{new Date(n.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                            </td>
                            <td className="py-3 pr-4">
                              <p className="text-white font-medium">{n.fullName}</p>
                              <p className="text-xs text-white/30">{n.email}</p>
                            </td>
                            <td className="py-3 pr-4">
                              <p className="text-white/70">{n.nominatorName || <span className="text-white/25 italic">—</span>}</p>
                              <p className="text-xs text-white/30">{n.nominatorEmail || ""}</p>
                            </td>
                            <td className="py-3 pr-4 text-white/60 text-xs max-w-[160px]">
                              {n.competitionId ? (compTitleMap[n.competitionId] || `#${n.competitionId}`) : <span className="text-white/25 italic">—</span>}
                            </td>
                            <td className="py-3 pr-4 text-right font-bold">
                              {n.amountPaid > 0
                                ? <span className="text-emerald-400">${(n.amountPaid / 100).toFixed(2)}</span>
                                : <span className="text-white/25">Free</span>
                              }
                            </td>
                            <td className="py-3 pr-4">
                              {n.referralCode
                                ? <span className="font-mono text-orange-400 text-xs bg-orange-500/10 px-1.5 py-0.5 rounded">{n.referralCode}</span>
                                : <span className="text-white/20 text-xs">—</span>
                              }
                            </td>
                            <td className="py-3 font-mono text-[10px] text-white/30 max-w-[140px] truncate">
                              {n.transactionId || <span className="italic text-white/15">no charge</span>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
